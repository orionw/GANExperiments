...
def train_discriminator(args, train_dataset, model, tokenizer, optimizer, scheduler):
    """ Train the model """
...
def evaluate_discriminator(args, model, tokenizer, eval_dataset, prefix=""):
...
def train_generator(args, train_dataset, model, tokenizer, optimizer, scheduler):
    """ Train the model """
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter()
...
def evaluate_generator(args, model, tokenizer, eval_dataset, prefix=""):
    # Loop to handle MNLI double evaluation (matched, mis-matched)
